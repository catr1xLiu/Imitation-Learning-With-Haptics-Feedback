...
class MultiCameraWrapper:
    def __init__(self):
        print("Setting up dummy cameras...")

        # Set dummy camera resolution
        self.robot_camera_resolution = (640, 480)
        self.user_camera_resolution = (640, 640)

    def set_trajectory_mode(self):
        # Dummy method to simulate starting cameras
        print("Dummy trajectory mode: cameras 'started'.")

    def stop_recording(self):
        # Dummy method to simulate stopping recording
        print("Dummy stop recording: cameras 'stopped'.")

    def read_cameras(self):
        # Create dummy black frames
        robot_frame = np.zeros((self.robot_camera_resolution[1], self.robot_camera_resolution[0], 3), dtype=np.uint8)
        user_frame = np.zeros((self.user_camera_resolution[1], self.user_camera_resolution[0], 3), dtype=np.uint8)
