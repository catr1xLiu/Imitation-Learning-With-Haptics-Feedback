...
env_urt = RobotAction(control_hz=5)
env_urt.reset()
print("Policy starting...")
from sim_env import BOX_POSE
import wandb 
wandb.login(key="66a1d6a287175272c885e586ddcb92759124ca90") #old login change if you want to switch which wandb project you push to
...
def main(args):
    set_seed(1)
    wandb.init(
    project="testingACT",  # change as needed
    config=args,
    name=f"trial_run_{args['seed']}",  # optional: useful for grouping runs
)
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                ...
                if 'images' in obs:
                    ...
                    # cv2.imshow("Rotated Robot Wrist Camera", obs['images']['wrist'])
                    # cv2.imshow("User Camera", obs['images']["POV"])
                else:
                ...
                actionSingle = action[:7]

                
                tcp_coor = actionSingle[:3].tolist()
                tcp_euler = actionSingle[3:6].tolist()
                gripper_state = actionSingle[-1]

                robot_action = np.concatenate([tcp_coor, tcp_euler, [1 - gripper_state]])

                comp_time = time_ms() - control_timestamps
                sleep_left = (1000 / 5) - comp_time
                if sleep_left > 0:
                    time.sleep(sleep_left / 1000)

                env_urt.send_action(action=robot_action, non_blocking=True)
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    # Use a single tqdm progress bar for epochs
    ...
    for epoch in epoch_bar:
        ...
        epoch_bar.set_postfix({
            "train_loss": f"{epoch_train_loss.item():.4f}",
        ...
        wandb.log({
            ...
            **{f"train_{k}": v.item() for k, v in epoch_summary.items()},
            **{f"val_{k}": v.item() for k, v in epoch_val_summary.items()},
        })
